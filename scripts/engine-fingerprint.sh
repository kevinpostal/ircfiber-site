...
if git rev-parse --git-dir >/dev/null 2>&1; then
  ...
  # Also hash current working tree in case of uncommitted engine changes (ignore generated version files)
  if ! git diff --quiet -- ':!common/source/ircfiber/build_version.d' ':!frontend/src/lib/buildInfo.ts' ':!public/dist' ':!backend/views/index.dt' -- engine common backend/source Containerfile 2>/dev/null; then
    HASH="${HASH}-dirty"
  fi
else
