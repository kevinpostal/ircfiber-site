...
# Guard: site repo must have frontend/ and backend/, must NOT have engine/
if [ ! -d "$ROOT/frontend" ] || [ ! -d "$ROOT/backend" ]; then
  echo "generate-version.sh: expected frontend/ and backend/ — are you in the site repo?" >&2
  exit 1
fi
if [ -d "$ROOT/engine" ]; then
  echo "generate-version.sh: found engine/ — this is the site repo, not engine" >&2
  exit 1
fi
...
COMMIT_URL=""
if [ "$HASH" != "unknown" ] && [ "$HASH" != "" ]; then COMMIT_URL="https://github.com/kevinpostal/ircfiber-site/commit/$HASH"; fi
