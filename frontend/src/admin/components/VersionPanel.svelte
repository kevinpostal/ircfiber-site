...
<Card title="Build Versions" subtitle="Gateway (serves the frontend from the same image) and engines — git commit, branch, built time">
  {#snippet actions()}
  ...
  {:else if data}
    {@const sync = (() => {
      ...
      return { engineSync, mostRecent };
    })()}
    <div class="space-y-6">
      {#if sync}
        {#if sync.mostRecent}
          <div class="rounded-md bg-surface-2 px-3 py-2 text-xs text-muted">Most recent build: {sync.mostRecent.name} {sync.mostRecent.commit?.slice(0,7)} at {fmtTime(sync.mostRecent.time)}</div>
        {/if}
      ...
      <div class="rounded-lg border border-border bg-surface-2 p-3">
        <div class="mb-2 text-xs font-semibold uppercase tracking-wider text-muted">Gateway <span class="normal-case text-[10px] text-muted">— vibe.d REST + WS + Svelte frontend</span></div>
