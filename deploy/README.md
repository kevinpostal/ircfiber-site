...
ZNC-era clients get ZNC's own command surface on top of that: `/znc <command>` (the raw line `ZNC …`) and `/msg *status <command>` — also any other `*module` target, e.g. `*controlpanel` — are answered by the bouncer itself and never reach the ircd, bound or unbound. Implemented: `Help`, `Version`, `ListNetworks`, `JumpNetwork <network>` (repoints a live connection: PARTs the old channels, replays 005 / channels / history for the new one), `AddNetwork <name> <host> [[+]port]`, `DelNetwork`, `Jump` (reconnect the bound network), `ListChans`, `Detach`/`Attach <#chan|*>`, `ListClients`. `<network>` is a name-slug, host or network id. `Detach` (and `PART #chan :detach`) is **persistent**: it sets `bncDetached` on that buffer's entry in the account's `bufferPrefs`, so the channel stays detached across reconnects, is listed under Settings → Bouncer → Networks, and fans out to the web app through the usual `pref_update` frame.

**Settings → Bouncer** reads everything in one `GET /api/me/bouncer`: endpoint, password plus its metadata (created, last used, last IP/clientid — derived from the per-account event trail in `irc:bnc:audit:<userId>`, LPUSH-capped at 50 rows with a 90-day TTL), every network's editable IRC identity, the known replay devices (`irc:bnc:seen:<networkId>`) and the last 20 events. Writes go to `POST /api/me/bouncer/settings` (partial object: `playbackLines`, `requireTls`, `allowedCidrs`, `maxClients`, `awayMessage` — stored in the prefs blob, clamped server-side) and `DELETE /api/me/bouncer/devices/:clientId` (forgets one device's replay cursor so its next attach replays the playback buffer). The access policy is enforced once per connection after authentication and before binding: no TLS when `requireTls`, an IP outside `allowedCidrs` and an account over `maxClients` are each answered with `ERROR :Closing link: …` and a `reject` row in the trail. A prefs-store read failure fails **open** so a Redis outage cannot lock a user out of their own bouncer. With `awayMessage` set, the bouncer sends `AWAY :<message>` upstream when an account's last client leaves a network and clears it on the next attach.

Per-network identity is `nick` / `ident` / `realName` on the network document. `ident` is the first `USER` parameter (`[A-Za-z0-9_.-]`, ≤10 chars — `USERLEN`); empty means "use the nick", which is byte-for-byte what pre-ident networks registered with. `ident` and `realName` only apply on the next (re)connect, so the tab offers **Reconnect now** after a change; the managed IRC Fiber network pins all three to the services account.
...
- **Adopt the sessions that outlived a restart**: the open-session map is per IRC session, so on (re)start FiberEye runs one `WHO * %tina,fe` (WHOX) and pairs the live network against the session rows that are still open — adopting them (so the eventual QUIT still yields a real duration), correcting the nick of anyone who renamed while it was away, and closing the rows whose client is provably gone with `quitInferred` (the quit instant is unknown, so the admin table says "unknown", never a fabricated `0s`). It never *creates* a row: a live client with no row is reported (`liveWithoutRow`) rather than invented, because a synthesised connect would inflate the rollup's `connects` and the flood counters. An ambiguous group — several unpaired clients behind one VPN exit — is counted, never guessed. `privs="users/auspex"` exists for this sweep alone: without it InspIRCd answers the WHOX IP field with the literal `255.255.255.255` and hides every `+i` user from a fuzzy WHO, leaving the pairing nick-only and blind to renames during the outage.
- **Persist every connect, nick change and quit** into Mongo (`fibereye_sessions`, `fibereye_ips`, `fibereye_bans`): the nick at connect *and* the nick in force now (`currentNick` on the session, `lastNick`/`lastNickAtMs` on the IP rollup, both moved by the `n`/`N` nick-change notice — the `seenicks` module is what emits it), ident, cloaked host, real IP, GECOS, connect class, port, TLS, NickServ account (from numeric `330`), the session duration once the quit arrives, and the display fields of the IP intelligence record below. Sessions age out after 90 days through a TTL index on `tsAt`; changing `IRCFIBER_FIBEREYE_RETENTION_DAYS` on a live deployment needs `db.fibereye_sessions.dropIndex("tsAt_1")` first (same for `ipintel_records` and `lastSeenAt_1`), because MongoDB refuses to re-create a TTL index with a different expiry (the role logs the conflict rather than failing).
